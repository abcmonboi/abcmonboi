...
const createMetaTag = asyncHandler(async (req, res) => {
  ...
  const metaTag = await MetaTag.create(req.body).catch((err,_message) => {
    // return message duplicate key if meta_url is duplicate
    console.log(err)
    if (err.code === 11000) {
      throw new Error("meta_url bị trùng");
    } else {
      throw err;
    }
  });
  ...
  return res.status(200).json({
    success: metaTag ? true : false,
    data: metaTag ? metaTag : "Something went wrong",
    mes: metaTag ? "Data created successfully" : "Data created failed",
  });
});
const getMetaTag = asyncHandler(async (req, res) => {
...
const getMetaTagByUrl = asyncHandler(async (req, res) => {
...
const getAllMetaTag = asyncHandler(async (req, res) => {
...
const updateMetaTag = asyncHandler(async (req, res) => {
  const { bid } = req.params;
...
const deleteMetaTag = asyncHandler(async (req, res) => {
  const { bid } = req.params;
